...
class OllamaClient:
    ...
    async def chat_async(self, messages: list[dict], timeout: int | None = None) -> str:
        """
        ...
        try:
            ...
            response = await asyncio.wait_for(loop.run_in_executor(None, self.chat, messages), timeout=request_timeout)
            return response
        except TimeoutError:
            log.error("LLM chat timeout after %d seconds", request_timeout)
            raise TimeoutError(f"LLM response timeout after {request_timeout} seconds")
        except Exception as e:
    ...
    async def chat_stream_async(self, messages: list[dict], timeout: int | None = None):
        """
        ...
        try:
            stream = await asyncio.wait_for(
                loop.run_in_executor(None, lambda: self.client.chat(model=self.model, messages=messages, stream=True)),
